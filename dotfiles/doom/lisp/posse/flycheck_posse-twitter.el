...
;; Add these variables to your my-posse.el
...
(defun my-send-tweet-from-buffer ()
  ...
  (let* ((content (buffer-substring-no-properties
    ...
    (cond
     ...
     (t
      ;; First, add to local microblog
      (my-add-to-microblog tweet-text media)

      ;; Then proceed with social media posting
          ...
          (let ((result (shell-command-to-string command)))
            ...
            (if (string-match-p "Successfully posted" result)
                (progn
                  ...
                  (kill-buffer))
              (message "Microblog: ✓ | Social: %s" result)))))))));; Keep your existing key binding
(map! :leader
      (:prefix ("t" . "Tweet")
