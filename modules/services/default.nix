...
let
  ...
  # --- IMPORTANT ---
  # Change 'enp1s0' to your server's actual primary network interface name.
  # You can find it by running the `ip a` command on your server.
  serverIP = (builtins.head config.networking.interfaces.enp1s0.ipv4.addresses).address;
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    services.caddy = {
      enable = true;
      virtualHosts = {
        # This syntax explicitly serves on port 80 (HTTP) and disables automatic HTTPS
