{ config, lib, pkgs, ... }:
{
  ...
  services = {
    ...
    # Lid switch and login manager configuration - REMOVE the extra "services."
    logind = {
      lidSwitch = "suspend";           # What to do when lid closes
      lidSwitchExternalPower = "suspend"; # Same behavior on AC power
      lidSwitchDocked = "ignore";      # Don't suspend when docked to external display
      powerKey = "suspend";            # Power button behavior
      powerKeyLongPress = "poweroff";  # Long press = shutdown
      extraConfig = ''
